...
- [x] 開発時のホットリロードで接続が増殖しないよう `global` キャッシュパターンで実装
...
- [x] `Account`, `Session`, `VerificationToken`（NextAuth Prisma Adapter 標準スキーマ）
...
- [x] `prisma/seed.ts` にサービス種別マスタ（配列定数）とテスト用 Admin ユーザーを投入
