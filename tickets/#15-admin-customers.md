...
- [x] `searchParams.q` でメール・氏名の部分一致検索
- [x] `CustomerTable`：氏名 / メール / 電話番号 / 案件数 / 登録日
- [x] 行クリックで詳細ページへ
...
- [x] 氏名 / メール / 電話番号 / 住所 / 登録日

履歴タブ（タブ UI を `_components` に実装）：

- [x] **問い合わせ履歴**：受付日 / 作業内容 / ステータス の一覧（詳細へのリンク付き）
- [x] **案件履歴**：作成日 / 作業内容 / ステータス の一覧（詳細へのリンク付き）
- [x] **請求履歴**：請求日 / 金額 / 支払状況 の一覧

担当メモ：

- [x] `CustomerMemoForm`：メモを保存する Server Action フォーム
  ```ts
  "use server"
